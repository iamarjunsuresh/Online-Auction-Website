...
@au=Auction.where("status=\'SCHEDULED\' and start_time<= :st and end_time >= :st",{st: ActiveSupport::TimeZone["UTC"].now.strftime('%Y/%m/%d %H:%M')})
      @au.each do |t|
   ...
   #   @au=Auction.where(:status=>'AUCTION_LIVE').where( :end_time<Time.now)
      ...
      @au=Auction.where("status=\'AUCTION_LIVE\' and end_time<= :st",{st: ActiveSupport::TimeZone["UTC"].now.strftime('%Y/%m/%d %H:%M')})
      @au.each do |t|
